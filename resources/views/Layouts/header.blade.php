...
$rejectedAttendanceRequests = collect();
$rejectedCount = 0;
...
$rejectedAttendanceRequests = AttendanceRequest::where('user_id', $userId)
->where('status', 'rejected')
->orderBy('work_date', 'desc')
->get();

$rejectedCount = $rejectedShiftMonths->count() + $rejectedAttendanceRequests->count();
}
        ...
        <div class="app-notification-container">
            ...
            <div id="notiDropdown" class="app-notification-dropdown">
                ...
                <ul class="app-notification-list">
                    ...
                    {{-- シフト差し戻し --}}
                    @foreach($rejectedShiftMonths as $yearMonth => $shifts)
                    @php
                    [$year, $month] = explode('-', $yearMonth);
                    @endphp

                    <li class="app-notification-item">
                        <span style="color:#e53935; font-weight:bold;">【シフト】</span>
                    ...
                    {{-- 打刻修正差し戻し --}}
                    @foreach($rejectedAttendanceRequests as $attendanceRequest)
                    <li class="app-notification-item">
                        <span style="color:#e53935; font-weight:bold;">【打刻修正】</span>
                        {{ \Carbon\Carbon::parse($attendanceRequest->work_date)->format('Y年n月j日') }}
                        の打刻修正申請が差し戻されました。<br>
                    </li>
                    @endforeach
